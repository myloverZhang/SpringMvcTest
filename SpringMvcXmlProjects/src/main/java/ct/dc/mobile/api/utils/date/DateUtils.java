package ct.dc.mobile.api.utils.date;
...
//        }
    ...
    public static boolean isDate(int date){
        ...
        String dateString = String.format("%d-%s-%s",date/10000,monthString,dayString);
    ...
    public static int DateToInt(Date date) {
        int intDate = getInteger(date,Calendar.YEAR)*10000+(getInteger(date,Calendar.MONTH)+1)*100+getInteger(date,Calendar.DAY_OF_MONTH);
        return intDate;
    }
    ...
    public static Date IntToDate(int intDate) {
        ...
        Date date = StringToDate(String.format("%d-%d-%d",intDate/10000,(intDate%10000)/100,intDate%100));
        return date;
    }
    /**
     ...
     * @return
     */
    public static int addDay(int date,int dayAmount){
        if (!isDate(date)){
            return 0;
        }
       return DateToInt(addDay(IntToDate(date),dayAmount));
    }
    /**
     * 增加日期的天数。失败返回null。
     * @param date 日期
